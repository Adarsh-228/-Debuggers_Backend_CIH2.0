...
def handle_connect():
    ...
    if tracker is None:
        print(f"Initializing new tracker with exercise: {current_exercise_type}")
        tracker = ExerciseTracker(current_exercise_type)
    else:
        print(f"Re-assigning exercise to existing tracker: {current_exercise_type}")
        tracker.exercise_type = current_exercise_type 
        tracker.reset_counter()
    emit('connection_ack', {'message': 'Connected! Select exercise & start.', 'current_exercise': current_exercise_type})
    ...
    if event_type == 'select_exercise':
        ...
        if exercise_type:
            ...
            if tracker:
                tracker.exercise_type = current_exercise_type
                tracker.reset_counter()
                print(f"Exercise changed to: {current_exercise_type} for client {request.sid}")
                emit('exercise_changed', {'message': f'Exercise set to {current_exercise_type}', 'current_exercise': current_exercise_type})
            else:
                # This case should ideally not happen if tracker is initialized on connect
                tracker = ExerciseTracker(current_exercise_type)
                print(f"Tracker was None, initialized with {current_exercise_type}")
                emit('exercise_changed', {'message': f'Tracker initialized. Exercise: {current_exercise_type}', 'current_exercise': current_exercise_type})
        else:
        ...
        if tracker is None:
            ...
            tracker = ExerciseTracker(current_exercise_type) # Fallback initialization
        ...
        try:
            ...
            landmarks_data, exercise_status, annotated_frame_b64 = tracker.process_frame_for_server(frame_bytes)
            ...
            emit('server_error', {'message': f'Base64 decoding error: {str(b64_error)}'})
        except Exception as e:
            print(f"Error processing frame: {e}")
            emit('server_error', {'message': f'Error processing frame: {str(e)}'})
    else:
